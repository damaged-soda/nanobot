...
```yaml
id: c5                          # 稳定 id（uuid 或短 hash）
job_id: b7c233fa                # 挂在哪个 cron job 上
text: "未来的新闻不再出现神经科学相关信息"
                                 # 内容是 prose，LLM 阅读 / 产出都不拆字段
origin: user_request            # user_request / llm_inference / system
status: active                  # active / revoked / superseded
created_at: 2026-04-23T10:00:00+08:00
source_trace_id: "xxx..."       # 创建时 LLM 那一轮的 trace_id
revoked_at: null
revoked_by: null                # 另一条 commitment id，如果是被替代
verification_history:
  - { run_at: ..., run_kind: simulate, verdict: pass,  detail: null }
  - { run_at: ..., run_kind: live,     verdict: fail,  detail: "..." }
```

**结构**固定的是外壳（id / job_id / origin / status / verification_history / ...）；**内容** `text` 是 prose。LLM 不预定义"append_exclusion"这种操作词汇——修改 = 新建 / revoke / supersede。

### 2. 持久化 + 加载

- 独立存储：`<workspace>/data/commitments.json`（或 `commitments/<job_id>.json` 每 job 一份）。**Step 1 侦察结束时确定**。
- 随 jobs.json 在同一 workspace 下。
- 读写走 `CommitmentStore` 对象，内部是 JSON 原子写（temp + rename）。

### 3. LLM 工具（3 个新 tool）

- `create_commitment(job_id: str, text: str, origin?: str = "user_request") -> commitment_id`
- `revoke_commitment(commitment_id: str, reason?: str) -> None`
- `list_commitments(job_id: str, status?: str = "active") -> list[Commitment]`

统一注册到 `ToolRegistry`，schema 按现有 tool 的样式（`tool_parameters_schema`）。不新建工具分类目录。

### 4. Prompt builder 注入

cron job 执行时（`_execute_job` 当前构造 prompt 的地方），读这个 job 上的 active commitments，把它们的 `text` 以一段固定模板拼进 prompt：

```
## Active commitments for this job
The following rules MUST be followed in this run. If any of them cannot be satisfied, say so explicitly in the output rather than silently violating.
...
现有的 payload.message 保留作为 job 自身的任务描述；commitments 是叠加的规则层。
...
- **不**引入 intent 类型层级（Phase 1 Step 2 加的 SendMessageIntent / NoReplyChosen 等在新方向下无消费者，见 `STATUS.md` 的代码清理说明）。
...
1. **侦察（read-only）**：
   - `nanobot/cron/service.py:_execute_job` 如何构造 prompt（在哪行 inject commitments 最小侵入）。
   - `nanobot/agent/tools/message.py` 的 send path 具体几行、最轻的 contextvar 插入位置。
   - jobs.json 的加载 / 存储模式，决定 commitments 存储跟在 workspace 的哪里。
   - 现有 ToolRegistry 注册 tool 的 pattern，对齐 API。
   - 产出对照表 `<file>:<line> → 改动点`。

2. **Commitment 数据模型 + store**：
   - `nanobot/commitments/`（新目录）：`models.py`、`store.py`。
   - `Commitment` dataclass + `CommitmentStore` CRUD（list/create/revoke）。
   - 原子文件写。
   - 单测。

3. **LLM 工具 3 个**：
   - 在 `nanobot/agent/tools/` 下新增 `commitment.py`。
   - 注册到 loop 启动时的 ToolRegistry。
   - 每个工具发对应 trace kind。
...
- 新建一个 `Commitment(job_id=..., text=...)` 后，下一次真 cron 运行的 prompt 里有这一条承诺（snapshot test）。
- `simulate_job_run(job_id)` 产出 `(outputs, verdicts)`；verdicts 对 commitments 逐条返回 pass/fail。
...
- 用户在 CLI 说"以后新闻不要神经科学"，系统不再落到 memory.md，而是产出一条 `Commitment` 并能在同一 turn 内 simulate 得到 verdict。
- `simulate_job_run` 可被 LLM 自发重复调用验证修改，每次都有 trace 可追。
- 一次"改承诺 → simulate 失败 → 改表述 → simulate 通过"的自循环能在集成测试里跑通。
- commitment store 的数据在进程重启后仍可读。
